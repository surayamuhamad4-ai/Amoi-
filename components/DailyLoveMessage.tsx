...
/**
 * components/DailyLoveMessage.tsx
 * Shows one message per calendar day, deterministically selected from
 * DAILY_MESSAGES using day-of-year modulo the list length — so the same
 * message shows all day, and it's identical on server and client render
 * (avoiding hydration mismatches that Math.random() would cause).
 */
...
export default function DailyLoveMessage() {
  ...
  return (
    <div
      ...
      aria-label="Today's love message"
    >
      <p className="font-hand text-2xl md:text-3xl text-rose-gold-dark text-center max-w-lg">
        {message}
      </p>
    </div>
  );
