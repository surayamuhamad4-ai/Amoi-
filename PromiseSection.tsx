 ...
 * The "promises" section — a set of vows/promises rendered as ribbon
 * cards, scroll-revealed one at a time.
 */
...
export default function PromiseSection() {
  ...
  return (
    <section
      id="promises"
      ref={ref}
      className="relative w-full bg-gradient-to-b from-lavender/20 to-cream px-6 py-24"
      aria-label="Promises"
    >
      <h2 className="font-display text-4xl md:text-5xl text-plum text-center mb-12">My Promises to You</h2>
      ...
      <div className="max-w-2xl mx-auto space-y-5">
        {PROMISES.map((promise, i) => (
          <motion.div
            ...
            className="flex items-start gap-4 bg-white/80 rounded-xl px-5 py-4 shadow-sm border border-lavender/30"
          >
            <span className="text-xl mt-0.5" aria-hidden="true">
              🎀
            </span>
            <p className="font-body text-plum/90 leading-relaxed">{promise}</p>
          </motion.div>
        ))}
      </div>
    </section>
