 ...
 * The centerpiece. Opens as a sealed envelope; clicking/tapping breaks
 * the seal and reveals the letter with a staggered paragraph reveal.
 * Content is the real letter — never replaced with placeholder text.
 */
...
export default function LoveLetter() {
  ...
  return (
    <section
      id="letter"
      className="relative min-h-screen w-full bg-cream flex flex-col items-center justify-center px-6 py-24"
      aria-label="Love letter"
    >
      <h2 className="font-display text-4xl md:text-5xl text-plum mb-12 text-center">
      ...
      <AnimatePresence mode="wait">
        {!isOpen ? (
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            className="group relative w-full max-w-md aspect-[3/2] rounded-lg bg-gradient-to-br from-rose-gold to-sakura shadow-xl shadow-rose-gold/30 flex items-center justify-center"
            aria-label="Open the sealed letter"
          >
            <div className="absolute inset-x-0 top-0 h-1/2 bg-gradient-to-b from-rose-gold-dark/40 to-transparent rounded-t-lg" />
          ...
          <motion.div
            ...
            className="relative w-full max-w-2xl bg-white/90 backdrop-blur rounded-2xl shadow-2xl shadow-plum/10 border border-rose-gold/20 px-6 py-10 md:px-12 md:py-14"
          >
            <p className="text-center font-hand text-xl text-rose-gold-dark mb-6" aria-hidden="true">
              for {GIRLFRIEND_NAME}
            </p>
            <div className="space-y-5 font-body text-plum/90 leading-relaxed text-[1.05rem] md:text-lg">
              {LETTER_PARAGRAPHS.map((para, i) => (
                <motion.p
                  ...
                  className={i === 0 ? "font-display text-xl text-plum" : ""}
                  lang="zh"
                >
                  {para}
                </motion.p>
              ...
              <motion.p
                ...
                className="font-hand text-2xl text-rose-gold-dark text-right pt-4"
              >
                永远爱你的，Aidil
              </motion.p>
            </div>
